...
hsv_max   = np.array([100,32,255])          #[H,S,V]:Sが距離に応じて増加(32~42)
count = 0
...
while True:
    ...
    imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)          # グレースケール変換
    ...
    if biggest.size != 0:
        ...
        # 出力結果にて表示する画像配列
        imageArray = ([img,imgGray,img_hsv,img_mask],
                      [imgContours,imgWarpColored, imgBinarization, imgMedian])
    ...
    # 出力結果の各ラベル
    lables = [["Original","Gray","HSV","masked"],
              ["Contours","Warp Prespective","Warp Binarization","Warp Median"]]
    ...
    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite("result/O_color"+str(count)+".jpg",imgWarpColored)
        cv2.imwrite("result/O_Binari"+str(count)+".jpg",imgBinarization)
        cv2.imwrite("result/O_Median"+str(count)+".jpg",imgMedian)
