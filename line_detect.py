...
IMAGE_PATH = "picture/corridor.png" # 読み込む画像

def main():
    image  = cv2.imread(IMAGE_PATH) # 画像読み込み
    image2 = cv2.imread(IMAGE_PATH) # 画像読み込み
    cv2.imwrite("result/LD_original.png", image)

    gray = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY) # グレースケール化
    outLineImage = cv2.Canny(gray, 50, 125, apertureSize = 3)   # 輪郭線抽出
    cv2.imwrite("result/LD_outLine.png", outLineImage)    # ファイル保存


    houghPList = hough_lines_p(image2, outLineImage)  # 確率的ハフ変換による直線抽出
    cv2.imwrite("result/LD_result_houghP.png", image2)        # ファイル保存
    draw_cross_points(image2, houghPList)             # 直線リストから交点を描画
    cv2.imwrite("result/LD_result_houghP_cross.png", image2)  # ファイル保存
